...
import csDomainColor from './domain-color-comp.wgsl';
...
const createComputePipeline = async (device: GPUDevice, texture: GPUTexture): Promise<ws.IPipeline> => {  
    let csShader = csDomainColor.concat(csFunc.concat(csColormap));  
...
export const run = async () => {
    ...
    gui.add(params, 'scale', 1, 20, 0.1);
    gui.add(params, 'functionSelection', 0, 12, 1);
